...
y devuelve varios resultados puntuados (0 a 5) con una heurística de
confiabilidad por sitio.
"""
...
class handler(BaseHTTPRequestHandler):
    def do_POST(self):
        ...
        if not api_key or not cx:
            self._responder(500, {"error": "Falta configurar GOOGLE_API_KEY o GOOGLE_CX"})
            return

        consulta = f'"{titulo}" "{artista}" acordes tablatura'

        try:
            resultados = self._buscar_en_google(consulta, api_key, cx)
        except Exception as e:
            self._responder(500, {"error": f"Error llamando a Google: {str(e)}"})
            return
    ...
    def _buscar_en_google(self, consulta, api_key, cx):
